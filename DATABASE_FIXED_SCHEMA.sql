-- Fixed Database Schema for TASUED Connect
-- This script fixes the foreign key constraint issues and properly handles UUID generation
...
-- Note: We don't drop the users table that extends auth.users, we modify it instead

-- 2. FIX THE USERS TABLE (without the foreign key constraint to auth.users since Supabase handles this differently)
-- The users table in the public schema should NOT have an explicit foreign key to auth.users
-- Supabase automatically handles the relationship via RLS policies

-- First, drop the current users table if it exists
DROP TABLE IF EXISTS public.users CASCADE;

-- Create the users table without explicit foreign key constraint
CREATE TABLE IF NOT EXISTS public.users (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- Use gen_random_uuid() for UUID generation
  email TEXT UNIQUE NOT NULL,
  first_name TEXT NOT NULL,
  last_name TEXT NOT NULL,
  other_names TEXT, -- Additional name field for full name support
  phone_number TEXT, -- Phone number field for contact
...
CREATE TABLE IF NOT EXISTS courses (
  ...
  name TEXT NOT NULL,
  description TEXT,
  lecturer_id UUID, -- Removed foreign key constraint to allow flexible user references
  department TEXT,
  level TEXT,
  semester TEXT,
  year INTEGER,
...
CREATE TABLE IF NOT EXISTS course_enrollments (
  ...
  student_id UUID, -- Removed foreign key constraint to allow flexible user references
...
-- 14. CREATE PROPER TRIGGERS FOR AUTO-USER CREATION
-- Function to handle new user creation (without explicit FK constraint issues)
...
AS $$
BEGIN
  INSERT INTO public.users (id, email, first_name, last_name, other_names, phone_number, role, is_active, is_email_verified)
  VALUES (
    NEW.id,
    NEW.email,
